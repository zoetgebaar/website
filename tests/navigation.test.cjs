...
assert.equal(new URL(t.nodes.destination).searchParams.get('v'),'20260923-5');
const demo=setup('[]','?demo=1');demo.nodes['.cart-pill'].click();assert.equal(new URL(demo.nodes.destination).searchParams.get('demo'),'1');
const html=fs.readFileSync('webshop.html','utf8');assert.match(html,/<a class="cart-pill show" href="bestellen.html\?v=20260923-5"/);
