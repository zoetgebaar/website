...
assert.equal(new URL(t.nodes.destination).searchParams.get('v'),'20260923-6');
const html=fs.readFileSync('webshop.html','utf8');assert.match(html,/<a class="cart-pill show" href="bestellen.html\?v=20260923-6"/);
assert.doesNotMatch(html,/aria-haspopup="dialog"/);
console.log('Passed: empty wishlist stays clickable; legacy button navigates; selection persists; current HTML uses a real order link.');
